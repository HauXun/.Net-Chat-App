...
{
	public class CCore : IActions, ITodo, ICommunicateSocket, ICCommunicate, IReceivePacket, ITranspotDefine
	{
		...
		public Client MyClient { get => m_myClient; private set { } }
		...
		public void Worker(object token = null)
		{
			...
			// fetch to server
			SendPacket(new TransactionPacket(0, "Hauxun logging"));
		}
		...
		public async void WaitForHandle(object token = null)
		{
			...
			try
			{
				while (m_client != null)
				{
					...
					receivePacketSizeTaskResult = await receivePacketSizeTask;
		...
		{
			SendPacket(new TransactionPacket(0, $"The client closed"));
		}
	}
}
