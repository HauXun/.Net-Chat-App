...
{
    public class SCore : IActions, ITodo, ICommunicateSocket, ISCommunicate, IReceivePacket, ISendPacket, ITranspotDefine
    {
        ...
        public void HandleAction(TransactionPacket packet, Socket socket)
        {
            switch (packet.Todo)
            {
                case (int)DoActions.Todo.PushLog:
                    m_userMsg += $"\r\n{(packet.Value as Client).Name} was logging";
                    foreach (Socket s in m_clientSockets)
                    {
                        if (s != null && s.Connected && socket != null && socket.Connected && s.RemoteEndPoint == socket.RemoteEndPoint)
                        {
                            m_clients.ForEach(x =>
                            {
                                if (x.ID == (packet.Value as Client).ID)
                                {
                                    x.M_Client = s;
                                }
                            });
                    ...
                    //SendAll(new TransactionPacket((int)DoActions.Todo.PushStatus, m_clients.Count));
                    foreach (Client client in m_clients)
                    {
                        SendAll(new TransactionPacket((int)DoActions.Todo.PushStatus, client));
                    }
                    SendAll(new TransactionPacket((int)DoActions.Todo.PushStatus, "send clients oke"));
                    break;
        ...
        public async void SendAll(TransactionPacket packet)
        {
            foreach (Socket client in m_clientSockets)
            {
                if (client != null && client.Connected)
                {
                    await SendPacket(packet, client);
                }
            }
        }
