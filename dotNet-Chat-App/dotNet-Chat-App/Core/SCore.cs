...
namespace dotNet_Chat_App.Core
{
	public class SCore : IActions, ITodo, ICommunicateSocket, ISCommunicate, IReceivePacket, ISendPacket, ITranspotDefine, IDelegateDefined
	{
		...
		public async void HandleAction(TransactionPacket packet, Socket socket)
		{
			...
			switch (packet.Todo)
			{
				...
				case (int)DoActions.MessageType.ClientToClient:
					...
					{
						if (x.ID == Convert.ToInt32(param[2]))
						{
						reSend:
							if (!MessageBLL.Instance.SaveMessage(packet, Convert.ToInt32(x.Online), Convert.ToInt32(param[2]), Convert.ToInt32(param[0])))
								goto reSend;

							if (x.M_Client != null && x.M_Client.Connected)
							{
								await SendPacket(FragmentationServices.Serialize(new TransactionPacket((int)DoActions.MessageType.ClientToClient, $"{param[1]}: {param[3]}")), x.M_Client);
							}
						}
					}
					break;
		...
		public async Task SendAll(TransactionPacket packet)
		{
			...
			{
				if (packet.Todo >= (int)DoActions.MessageType.ServerSendAll)
				{
					//m_clients.ForEach(x =>
					//{
					//reSend:
